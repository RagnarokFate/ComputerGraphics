...
void RenderFrame(GLFWwindow* window, Scene& scene, Renderer& renderer, ImGuiIO& io)
{
	...
	if (frameBufferWidth != renderer.GetViewportWidth() || frameBufferHeight != renderer.GetViewportHeight())
	{
		...
		scene.GetActiveCamera().Ratio = frameBufferWidth / frameBufferHeight;
		...
		////renderer.~Renderer();
		//view_width = frameBufferWidth, view_height = frameBufferHeight;
		...
		// TODO: Set new aspect ratio
			...
			if (io.KeysDown[65])// move with 'a'
			{
				...
				{
						vec3 currVec = vec3(scene.GetActiveModel().MeshModelTransformation.GetTranslateBar());
						currVec.x -= 50;
						scene.GetModel(scene.GetActiveModelIndex()).MeshModelTransformation.SetTranslateBar(currVec);
			...
			if (io.KeysDown[88] || io.KeysDown[120])
			{
				...
				{
					scene.GetActiveModel().MeshModelTransformation.SetRotateBar(scene.GetActiveModel().MeshModelTransformation.GetRotateBar() + vec3(5.0f, 0.0f, 0.0f));
				}
			}
			if (io.KeysDown[89] || io.KeysDown[121])
			{
				if (scene.GetModelCount())
				{
					scene.GetActiveModel().MeshModelTransformation.SetRotateBar(scene.GetActiveModel().MeshModelTransformation.GetRotateBar() + vec3(0.0f, 5.0f, 0.0f));
				}
			}
			if (io.KeysDown[90] || io.KeysDown[121])
			{
				if (scene.GetModelCount())
				{
					scene.GetActiveModel().MeshModelTransformation.SetRotateBar(scene.GetActiveModel().MeshModelTransformation.GetRotateBar() + vec3(0.0f, 0.0f, 5.0f));
				}
			}
...
void DrawImguiMenus(ImGuiIO& io, Scene& scene)
{
	...
	//Main Window
	{
		...
		if (ImGui::CollapsingHeader("Other"))
		{
			...
			ImGui::Checkbox("Stats Window", &Stats);      // Edit bools storing our window open/close state
	...
	if (MeshModelFeatures)
	{
		...
		if (scene.GetModelCount())
		{
			...
			MeshModel& CurrentMeshModel = scene.GetModel(Model_Number);
			...
			if (ImGui::CollapsingHeader("Fill Triangle Settings"))
			{
				...
				ImGui::Checkbox("Triagnle Fill", &CurrentMeshModel.TriangleFill);
				if (CurrentMeshModel.TriangleFill)
				{
					ImGui::RadioButton("ScanLine", &CurrentMeshModel.TriangleFillMode, ScanLine);
					ImGui::RadioButton("Walking Edge", &CurrentMeshModel.TriangleFillMode, WalkingEdge);

					if (CurrentMeshModel.TriangleFillMode == ScanLine)
					{
						static int index = 0;

						const char* items[] = { "Black & White", "GrayScale" , "ColorFul" };
						static const char* current_item = "Black & White";
						if (ImGui::BeginCombo("SL Combo", current_item, 1))
						{
						...
						CurrentMeshModel.ScanLineMode = index;
					}

					if (CurrentMeshModel.TriangleFillMode == WalkingEdge)
					{
						static int index = 0;

						const char* items[] = { "Constant", "Random", "Constant with depth caulcation"};
						static const char* current_item = "Constant";

						if (ImGui::BeginCombo("WE Combo", current_item, 1))
						{
							for (int n = 0; n < IM_ARRAYSIZE(items); n++)
							{
								bool is_selected = (current_item == items[n]);
						...
						CurrentMeshModel.WalkingEdgeMode = index;
					}
	...
	if (TransformationWindow)
	{
		...
		if (ImGui::CollapsingHeader("Local Transformation"))
		{
			...
			if (scene.GetModelCount())
			{
				...
				MeshModel Mesh_Model = scene.GetModel(Model_Number);
		...
		if (ImGui::CollapsingHeader("World Transformation"))
		{
			...
			if (scene.GetModelCount())
			{
				...
				ImGui::SliderFloat3("World Translation", (float*)&WTranslateBar, -400, 400);
	...
	if (Camera_Projection)
	{
		...
		if (ImGui::CollapsingHeader("Projection Type"))
		{
			...
			if (CurrentCamera.ProjectionStatus == 1)
			{
				...
				CurrentCamera.SetCameraPerspective(FOV, (float)view_width, (float)view_height, Z_Near, Z_Far);
